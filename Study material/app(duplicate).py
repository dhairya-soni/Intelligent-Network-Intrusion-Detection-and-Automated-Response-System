...
def determine_severity(detection_result):
    """Determine alert severity with balanced thresholds"""
    score = detection_result['ml_score']  # 0-1 scale
    rule = detection_result['rule_matched']
    
    # If rule matched, bump up severity
    if rule:
        if score > 0.65:
            return SEVERITY_CRITICAL  # Rule + High ML = CRITICAL
        elif score > 0.45:
            return SEVERITY_HIGH      # Rule + Medium ML = HIGH
        else:
            return SEVERITY_MEDIUM    # Rule + Low ML = MEDIUM
    
    # No rule match - pure ML score
    else:
        if score > 0.80:
            return SEVERITY_HIGH      # Very high anomaly
        elif score > 0.60:
            return SEVERITY_MEDIUM    # Moderate anomaly
        elif score > 0.40:
            return SEVERITY_LOW       # Mild anomaly
        else:
            return None  # Too low to alert (normal traffic)
